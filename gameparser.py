...
def filter_words(words, skip_words):
    ...
    if type(words) == list:
        ...
        wordlist = [word for word in new_words.split() if word not in skip_words]
        return wordlist

    wordlist1 = [word for word in words.split() if word not in skip_words]
    return wordlist1
...
def normalise_input(user_input):
    ...
    x = filter_words(no_punct, skip_words)
